...
function convertAggregate(aggregate, schema) {
	...
	let metrics = buildMetrics(aggr);
	let buckets = buildBuckets(aggr, schema);
...
function buildBuckets(aggregate, schema) {
	let aggr = aggregate.getData();
	if (_.isEmpty(aggr.groupBy)) { return null; }

	let factory = aggregate.getAggregateFactory();
	let groupByAggregateType = factory.getAggregatType('group-by');
	let buckets = {};
	for (let i = 0, len = aggr.groupBy.length; i < len; ++i ) {
		let group = aggr.groupBy[i];
		let key = `group_${i}`;
		let groupByType = groupByAggregateType._findGroupByType(group);
		...
		if (name === 'field') {
			...
			if (_.isEmpty(buckets.ranges)) { continue; }
			let isDate = _.isDate(buckets.ranges[0]);
			let ranges = _.map(buckets.ranges, (range) => {
				let r = {};
				if (range.start) {
