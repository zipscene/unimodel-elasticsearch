 ...
 */
function convertAggregate(aggregate, prefix) {
	...
	// throw an exception.
	let metrics = buildMetrics(aggregate, prefix);
	let buckets = buildBuckets(aggregate, prefix);
	...
	for (let field in buckets) {
		...
		bucket.aggregations = objtools.deepCopy(metrics);
	}
	return buckets;
}
exports.convertAggregate = convertAggregate;

function buildBuckets(aggregate, prefix) {
...
function buildMetrics(aggregate, prefix) {
	let aggr = aggregate.getData();
	...
	for (let field in aggr.stats) {
		metrics[`${prefix}_${field}`] = { stats: { field } };
	}
	return metrics;
}
