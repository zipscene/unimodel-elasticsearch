...
class ElasticsearchConnection extends EventEmitter {
	...
	constructor(clientOptions={}, indexConfigs={}, indexOptions={}, options={}) {
		...
		// for this connection.
		this._indexManager = new ElasticsearchIndexManager(this, indexConfigs, indexOptions);
	...
	setIndexConfigs(indexConfigs) {
		this._indexManager.indexConfigs = indexConfigs;
	}
	...
	setIndexOptions(indexOptions) {
		this._indexManager.indexOptions = indexOptions;
	}
	...
	_tryElasticsearchConnect() {
		return this._client.cluster.health({ level: 'cluster', waitForStatus: 'green' })
	 ...
	 *
	 * @method initialize
	 * @return {Waiter} - Resolve with initizlied ElasticsearchConnection.
	 */
	initialize() {
		if (this._client) {
		...
		let clientOptions = objtools.merge({
			...
			defer: () => {
				...
				// Automatically convert, then rethrow an error from ElasticSearch
				p.promise.catch((err) => { throw ElasticsearchError.fromESError(err); });
				return p;
			}
		}, this._clientOptions);
		// Initialize a new client
		this._client = new elasticsearch.Client(clientOptions);
	...
	_getClientConnection() {
		return new Promise((resolve, reject) => {
			this._client.transport.connectionPool.select((err, connection) => {
	 ...
	 *
	 * @return {elasticsearch.Client} - The underlying ElasticSearch Client.
	 */
	getClient() {
		return this._client;
	}
