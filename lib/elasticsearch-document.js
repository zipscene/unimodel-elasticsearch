			...
			.then(() => {
				...
				return this.model.getMapping().then((mapping) => {
					if (mapping._parent && !this.getParentId()) {
						throw new XError(XError.INTERNAL_ERROR, 'Parent ID is required for child models.');
					}
				});
			})
	...
	remove() {
		return Promise.resolve()
			.then(() => {
				...
				return this.model.getMapping().then((mapping) => {
					if (mapping._parent && !this.getParentId()) {
						throw new XError(XError.INTERNAL_ERROR, 'Parent ID is required for removing child models.');
					}
				});
			})
