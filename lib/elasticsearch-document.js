...
class ElasticsearchDocument extends SchemaDocument {
	...
	/**
	 * Create an ElasicsearchDocument from raw Elasticsearch Document data.
	 *
	 * @method fromESData
	 * @static
	 * @return {ElasticsearchDocument} - The ElasticsearchDcoument representing the raw data.
	 */
	static fromESData(model, esdata) {
		...
		return new ElasticsearchDocument(model, esdata._source, fields, true);
	}
	...
	save() {
		...
		return Promise.resolve()
			.then(() => {
				// Ensure this document can actually be saved
				return this.model.getMapping().then((mapping) => {
					if (mapping._parent && !this.getParentId()) {
						throw new XError(XError.DB_ERROR, 'Parent ID is required for child models.');
					}
				});
			})
		...
		return Promise.resolve()
			.then(() => {
				...
				if (this._originalData === null || this._originalFields === null) {
					...
					throw new XError(XError.DB_ERROR, msg);
				}
				...
				return this.model.getMapping().then((mapping) => {
					if (mapping._parent && !this.getParentId()) {
						throw new XError(XError.DB_ERROR, 'Parent ID is required for removing child models.');
					}
				});
			})
