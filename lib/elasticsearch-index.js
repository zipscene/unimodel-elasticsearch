...
class ElasticsearchIndex extends EventEmitter {
	...
	initialize() {
		...
		this.connection.getClient().then((client) => {
			...
			return client.indices.exists({ index: this.name }).then((exists) => {
				if (!exists) {
					// Create the index and set/create settings/warmers/aliases
	...
	/**
	 * Initialize an existing client by putting settings/warmers/aliases on the index.
	 *
	...
	_initializeExistingIndex(client) {
		return client.indices.putSettings({
			...
			let aliases = _.pairs(this.config.aliases);
			return pasync.eachSeries(aliases, ([ name, alias ]) => {
