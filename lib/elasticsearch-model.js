	...
	findStream(query, options = {}) {
		...
		Promise.all([ // Ensure the ES Client and Index we're hittings are available
			...
			.then(([ client, index ]) => {
				let isDone = false;
				let progress = 0;
				let limit = _.isNumber(options.limit) ? options.limit : -1;
				let scrollId = null;
				return pasync.whilst(() => {
					...
					return scrollPromise.then((resp) => {
						...
						if (scrollId && !hits.length) {
							// We are done getting results (NOTE: scan queries don't return results immediately)
							isDone = true;
							return Promise.resolve();
						}
						...
						return pasync.each(hits, (hit) => new Promise((resolve, reject) => {
							let doc = this._createExisting(hit, !_.isEmpty(options.fields));
							pass.write(doc, (err) => {
