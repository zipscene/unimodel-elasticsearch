...
class ElasticsearchModel extends SchemaModel {
	...
	aggregateMulti(query, aggregates, options = {}) {
		...
		let esaggregatesArray = [];
		for (let i = 0, len = aggregates.length; i < len; i++) {
			aggregates[i] = this.normalizeAggregate(aggregates[i]);
			esaggregatesArray[i] = convertAggregate(aggregates[i], `${i}`);
		}
		let esaggregates = objtools.merge({}, ...esaggregatesArray);
		...
		return Promise.all([ // Ensure the ES Client and Index we're hittings are available
			...
			.then(([ client, index ]) => {
				...
				aggrParams.size = 0;
				aggrParams.body.aggregations = esaggregates;
			...
			.then((resp) => {
				...
				let result = [];
				for (let key in resp.aggregations) {
					let i = parseInt(key.split('_')[1]);
					if (isNaN(i)) { throw new XError(XError.INTERNAL_ERROR, 'Error processing aggregate results'); }
					result[i] = convertAggregateResult(resp.aggregations[key], aggregates[i], this.getSchema());
				}
				return result;
			});
	}
