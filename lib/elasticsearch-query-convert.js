	...
	for (let exprKey in operator) {
		...
		try {
			if (exprKey === '$text') {
				...
				esFilter = { regexp: {
					[`${resultKey(resultKeyPrefix, ensureExactMatchField(model, queryKey))}`]: wildcardRegex
				} };

			} else if (exprKey === '$not') {
				let nottedExprs = operatorExpressionToFilters(queryKey, exprVal);
				esFilter = { bool: {
				...
				let nestedSchemaData = model.getSchema().createSubschema(queryKey);
				if (!nestedSchemaData || !nestedSchemaData.nested) {
				...
				let [ nestedFilter, nestedErrors ] = elasticsearchQueryConvert(exprVal, model, nestedResultKeyPrefix);
				...
				if (operator.$maxDistance) {
					nearMaxDistance = operator.$maxDistance;
				} else {
			...
			if (key[0] === '$') {
				...
				if (key === '$and' || key === '$or' || key === '$nor') {
					...
					for (let childType in queryVal) {
						if (childType[0] === '$') {
							let msg = `Unexpected query operator in ${key} value: ${chidlType[0]}`;
							throw new QueryValidationError(msg, { query });
						}
						let childModel;
						try {
				...
				andFilters.push({ term: {
					[`${resultKey(resultKeyPrefix, ensureExactMatchField(model, key))}`]: queryVal
				} });
			}
		} catch(err) {
			errors.push(err);
...
function elasticsearchQueryConvert(query, model) {
	let [ filter, errors ] = queryExpressionToFilter(query, model);
...
function getFieldIndexes(model, field) {
	...
	try {
		...
		throw new QueryValidationError(`Could not find field ${field} in schema`, { model, field }, err);
	}
